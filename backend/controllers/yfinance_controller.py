...
async def get_stock_price(symbol: str):
    """
    Get the real-time stock price for the requested symbol.
    """
    try:
        price_data = yfinance_service.get_realtime_stock_data(symbol)
        ...
        logger.error("Error fetching stock price for %s: %s", symbol, error)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch stock price: {error}",
        )
...
@router.get(
    ...
    description=(
        "Fetch the closing price for a stock on a specific date (or the most recent "
        "trading day before that date)."
    ),
)
async def get_historical_price(
    ...
    """
    Get the closing price of a stock on or before the given date.
    """
    try:
        try:
